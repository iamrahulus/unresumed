      ...
      <CardContent className="grid md:grid-cols-3 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={() => onSchedule(cand)}>
              <Calendar className="h-4 w-4 mr-2"/> Schedule
            </Button>
            <Button size="sm" variant="secondary" onClick={() => toast.success("Pushed to ATS (CSV)") }>
        ...
        <Tabs defaultValue="discover" className="w-full">
                    ...
                    <CardContent className="space-y-4">
                      {Object.entries(DEFAULT_WEIGHTS).map(([k,v]) => (
                        <div key={k} className="space-y-1">
                          ...
                          <Slider value={[Math.round(v*100)]} onValueChange={(arr)=>{/* static for demo */}} step={5} min={0} max={100} />
                  ...
                  <Card className="border-dashed">
                    ...
                    <CardContent className="space-y-3 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch id="claimed" checked={false} onCheckedChange={()=>{}} />
                      </div>
                      <div>
                        <Label>Must-have skills</Label>
                        <div className="mt-2 grid grid-cols-2 gap-2" style={{maxHeight:160, overflow:'auto', paddingRight:4}}>
                          {ALL_SKILLS.map((s) => (
                            <label key={s} style={{display:'flex', gap:8, fontSize:12}}>
          ...
          <TabsContent value="shortlist" className="mt-4 space-y-4">
            {shortlist.map(({cand, score}) => (
              <CandidateRow key={cand.id} cand={cand} roleTokens={roleTokens} weights={DEFAULT_WEIGHTS} hardFilters={{onlyClaimed:false,mustHaveSkills:[]}} onSchedule={handleSchedule} />
            ))}
          </TabsContent>
          ...
          <TabsContent value="fairness" className="mt-4">
            <div className="grid md:grid-cols-3 gap-6">
              <FairnessPanel visibleCount={MOCK_CANDIDATES.length} totalCount={MOCK_CANDIDATES.length} />
              <Card>
                <CardHeader className="py-3">
